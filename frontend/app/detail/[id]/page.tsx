...
import PlayButtons from "@/components/PlayButton";
...
export async function generateMetadata({ params }: VideoDetailProps): Promise<Metadata> {
  ...
  const title = `${video.vod_name} - 播放详情`;
  const description = `当前 ${video.vod_name}播放详情,  ${video.type_name} 分类下的视频.`;
...
export default async function VideoDetail({ params }: VideoDetailProps) {
  ...
  const websiteData = data.data.attributes;
  const API_BASE_URL = websiteData.API_BASE_URL;
  const playLinks = data.data.attributes.PlayLink;
  ...
  <div className="flex flex-col md:flex-row">
    ...
    <div className="md:w-1/2">
      <Image src={video.vod_pic} alt={video.vod_name} width={640} height={360} className="rounded-lg" />
    </div>

    {/* 视频信息 */}
    <div className="mt-4 md:mt-0 md:w-1/2 md:pl-8">
  <h1 className="text-2xl font-bold">{video.vod_name}</h1>
  <p className="text-gray-500 mt-2">分类: {video.type_name}</p>
  <p className="text-gray-500 mt-1">更新时间: {video.vod_time}</p>
  
  {/* 显示提取后的标签 */}
  <div className="mt-4">
    <p className="text-gray-500 font-bold">标签:</p>
  ...
  {/* 播放按钮 */}
  <PlayButtons videoId={video.vod_id} playLinks={playLinks} />
</div>
